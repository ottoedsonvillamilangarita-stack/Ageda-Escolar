...
def mostrar_perfil(usuario_data):
    """Muestra el perfil del usuario con opciones de edición"""
    ...
    with tab1:
        ...
        nuevo_email = st.text_input("Correo electrónico", value=email if email != "No registrado" else "")
        nuevo_telefono = st.text_input("Teléfono", value=telefono if telefono != "No registrado" else "")
        ...
        if st.button("💾 Actualizar datos"):
            ...
            if datos_actualizar:
                url_update = f"{SUPABASE_URL}/rest/v1/personas?id_persona=eq.{usuario_data['id_persona']}"
