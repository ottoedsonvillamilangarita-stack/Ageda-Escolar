    ...
    if not st.session_state.recover_mode:
        ...
        with col1:
            if st.button("Ingresar", use_container_width=True):
                # Consultar Supabase
                url = f"{SUPABASE_URL}/rest/v1/usuarios_login?username=eq.{usuario}&password_hash=eq.{password}"
                headers = get_headers()
        ...
        st.divider()
        st.caption("Usuarios de prueba: estudiante.laura, docente.herrera, secretaria")
        ...
        with col1:
            if st.button("Enviar instrucciones", use_container_width=True):
                ...
                if response.status_code == 200 and response.json():
                    ...
                    st.info("En la versión real, recibirías un correo para cambiarla.")
                else:
